...
def extract_parameters(message: str, current_state: dict) -> dict:
    ...
    # Check for 3 digit number starting with 1 (typical height in cm) or format 1.7m / 1.70m
    ...
    if not weight_match:
        # Fallback to loose number if preceding with "nặng"
    ...
    if budget_match:
        ...
        # If user wrote 50 instead of 50000, convert if they used 'k' or 'ngàn' or if it is low
        if val < 1000:
            state['budget'] = val * 1000.0
        else:
            state['budget'] = float(val)
    else:
        # Check plain numbers that look like money (e.g., 50000, 70000)
...
def get_rule_response(message: str, state: dict) -> tuple[str, dict]:
    ...
    # First, try to extract any parameters from current message
    updated_state = extract_parameters(message, state)
    
    # Check if we gained new values
    gained_new = False
    for k, v in updated_state.items():
        if state.get(k) != v:
            gained_new = True
            
    # Check if we have all necessary parameters for calculation
    # Needed: height, weight, age, gender, budget (activity defaults to sedentary if not provided)
    missing = []
    ...
    # If we have all required values, perform the full calculation and recommendation!
    if not missing:
        ...
        # Reset parameters in session after a successful recommendation to start fresh next time
        # but keep it if they want to ask follow-up questions
        return resp, updated_state
        
    # If some variables are present but not all, nudge them to input the missing ones
...
def get_chatbot_response(message: str, state: dict) -> tuple[str, dict]:
    """
    Core entry point for chatbot.
    If GEMINI_API_KEY is available, we call Gemini for a personalized, intelligent answer.
    Otherwise, we use the local rule-based system.
    """
    gemini_key = os.environ.get("GEMINI_API_KEY", "")
    openai_key = os.environ.get("OPENAI_API_KEY", "")
    
    # Parse parameters locally to maintain context state anyway
    state = extract_parameters(message, state)
    
    # If keys are missing, default to local rule engine (which is very good)
    ...
    # If OpenAI is configured (as alternative)
    if openai_key:
        try:
    ...
    # Fallback to local rule engine if both AI APIs fail
    return get_rule_response(message, state)
